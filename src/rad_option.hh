...
class radius_mono {
	protected:
		...
		inline void r_init(int ijk,int s) {(void)ijk;(void)s;}
		/** Sets a required constant to be used when carrying out a
		 * plane bounds check. */
		inline void r_prime(double rv) {(void)rv;}
		...
		inline double r_current_sub(double rs,int ijk,int q) {(void)ijk;(void)q;return rs;}
		 ...
		 * otherwise. */
		inline bool r_scale_check(double &rs,double mrs,int ijk,int q) {(void)ijk;(void)q;return rs<mrs;}
};
